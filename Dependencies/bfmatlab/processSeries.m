function [result] = processSeries(id, settings, varargin)
...
for s = 1:numSeries
    ...
    result2 = seriesMetadata;
    result3 = colorMaps;
    result4 = r.getMetadataStore();
    
    T = r.getSizeT();
    Z = r.getSizeZ();
    
    hyperstack = reshape(result1(:,1),[Z,T])';
    
    name = imageList{1,2};
    idx = strfind(name,';');
    filename = name(idx(1)+2:idx(2)-1);
    idx2 = strfind(filename, '"');
    filename(idx2) = [];
    
    
    if isempty(filename)
        filename = ['series ' num2str(2)];
    end
    
    for t = 1:T
        hypererStack(:,:,:,t) = cat(3, hyperstack{t, :});
    end
    
    if settings.saveTiffStack
        warning('Tiff stack writing not built in')
    end
    
    if settings.saveZProjection
        writeTiff(squeeze(max(hypererStack,[],3)), [settings.dirZProj filename '.tif'])
    end
    
    if settings.saveMP4
        writeMP4(squeeze(max(hypererStack,[],3)), [settings.dirMP4 filename])
    end
